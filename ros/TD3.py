...
"""
ROS1 TD3 actor node aligned with scenarios/gaplock_td3.yaml.
...
class RLActorNode:
    def __init__(self, default_ckpt: str = "td3_gaplock.pt") -> None:
        ...
        if not os.path.isfile(self.ckpt_path):
            rospy.logfatal("Checkpoint not found: %s", self.ckpt_path)
        ...
        rospy.loginfo("TD3 ROS actor ready | ckpt=%s | device=%s", self.ckpt_path, self.device)
