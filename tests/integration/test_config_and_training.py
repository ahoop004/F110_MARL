...
class TestTrainingLoop:
    ...
    def simple_setup(self):
        ...
        # Use fixed obs_dim that works with the environment
        agent_config = {
            "obs_dim": 50,  # Fixed size for testing
            "act_dim": 2,
            "action_low": np.array([-1.0, -1.0]),
            "action_high": np.array([1.0, 1.0]),
            "device": "cpu",
    ...
    def test_training_loop_runs_episodes(self, simple_setup):
        ...
        assert "agent_0" in training_history
        assert len(training_history["agent_0"]) == 5
        # Check that each episode has the expected keys
        for ep_stats in training_history["agent_0"]:
    ...
    def test_multi_agent_training_loop(self):
        ...
        for i in range(2):
            ...
            agents[f"agent_{i}"] = AgentFactory.create("ppo", agent_config)

        loop = TrainingLoop(env, agents, max_episodes=3, max_steps_per_episode=30)
        training_history = loop.run()

        assert training_history is not None
        assert len(training_history["agent_0"]) == 3
        assert len(training_history["agent_1"]) == 3
...
class TestCheckpointResume:
    ...
    def test_checkpoint_save_and_load(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert (checkpoint_path / "agent_0.pt").exists()

            # Create new agent and load
            new_agent = AgentFactory.create("ppo", config)
            new_agents = {"agent_0": new_agent}
    ...
    def test_multiple_agent_checkpoint(self):
        ...
        for i in range(3):
            ...
            agents[f"agent_{i}"] = AgentFactory.create("ppo", config)
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for i in range(3):
                assert (checkpoint_path / f"agent_{i}.pt").exists()

            # Load into new agents
            new_agents = {}
            for i in range(3):
                new_agents[f"agent_{i}"] = AgentFactory.create("ppo", configs[i])
...
class TestEdgeCases:
    ...
    def test_zero_episodes_training(self):
        ...
        env_config = {"map": "maps/line_map.yaml", "num_agents": 1}
        env = EnvironmentFactory.create(env_config)
        ...
        loop = TrainingLoop(env, {"agent_0": agent}, max_episodes=0)
        training_history = loop.run()

        assert training_history is not None
        assert len(training_history.get("agent_0", [])) == 0
    ...
    def test_single_step_episode(self):
        ...
        loop = TrainingLoop(env, {"agent_0": agent}, max_episodes=2, max_steps_per_episode=1)
        training_history = loop.run()

        assert len(training_history["agent_0"]) == 2

    def test_mixed_algorithm_multi_agent(self):
        """Test multi-agent with different algorithms."""
        env_config = {"map": "maps/line_map.yaml", "num_agents": 2}
        env = EnvironmentFactory.create(env_config)
        ...
        agents = {
            "agent_0": AgentFactory.create("ppo", ppo_config),
            "agent_1": AgentFactory.create("td3", td3_config),
        }
