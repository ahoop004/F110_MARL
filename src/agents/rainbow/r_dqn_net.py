...
class NoisyLinear(nn.Module):
    """Factorised Gaussian noisy linear layer as introduced in Rainbow."""
...
class RainbowQNetwork(nn.Module):
    """Dueling network with distributional head and optional noisy exploration."""
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        if atoms <= 1:
            raise ValueError("RainbowQNetwork requires at least two atoms for the support")
        if n_actions <= 0:
    ...
    def dist(self, obs: torch.Tensor) -> torch.Tensor:
        """Return categorical action-value probabilities."""
