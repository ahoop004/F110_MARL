"""Rainbow DQN agent with noisy nets, n-step returns, and distributional critics."""
...
from f110x.policies.r_dqn.net import RainbowQNetwork
...
class RainbowDQNAgent:
    ...
    def __init__(self, cfg: Dict[str, Any]):
        ...
        if self.atoms <= 1:
            raise ValueError("RainbowDQN requires at least two atoms (categorical support)")
        if self.v_max <= self.v_min:
            raise ValueError("RainbowDQN requires v_max > v_min for categorical support")
        ...
        self.use_epsilon = bool(cfg.get("epsilon_enabled", not self.use_noisy))
        ...
        self.epsilon_decay_rate = float(cfg.get("epsilon_decay_rate", 0.0 if self.use_epsilon else 0.0))
